...
float disp(in vec2 p) {
    return scmin.y + 0.1 + hash2(p).x * 0.5 + texture2D(iChannel0, vec2(hash2(p).x, 0.0)).r*2.0;
}
...
void main(void) {
    ...
    CameraOrbitRay(gl_FragCoord.xy, 1.0, vec3(0.0), 10.0, ro, rd, t);

    gl_FragColor = vec4(pow(Render(ro, rd, LightDir(t)), vec3(0.5454)), 1.0);
}